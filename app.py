...
df = pd.read_csv(r'C:\Users\natan\Dropbox\PC\Downloads\personalized product dataset.csv', names=columns, nrows=1_000)
...
# Convert all data in ratings column to numeric and handle missing values
df['ratings'] = pd.to_numeric(df['ratings'], errors='coerce')
df = df.dropna(subset=['ratings'])  # Drop rows with NaN ratings
...
if df_final.empty:
    print("Error: df_final is empty. No users have rated 15 or more items.")
else:
    ...
    # Apply the function to the 'ratings' column to create a new 'sentiment' column
    ...
    # Continue with the rest of your data processing and recommendation logic...
    # Splitting the dataset into train and test data with a test size of 20%
    train_data, test_data = train_test_split(df_final, test_size=0.2, random_state=0)

    # Grouping the train data by 'productId' and aggregating the count of 'userId'
    ...
    # Sorting the train_data DataFrame by 'score' and 'productId'
    train_data               
